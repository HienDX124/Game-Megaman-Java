...
public class Megaman extends Human {
    ...
    public void jump() {
        ...
        if (!getIsJumping()) {
            ...
            else if (getGameWorld().physicalMap.havaCollisionWithLeftWall(rectLeftWall) != null && getSpeedX() < 0) {
                setSpeedY(-5.0f);
                // setSpeedX(1);
                flyBackAnim.reset();
                flyForwardAnim.reset();
    ...
    public void attack() {
        if (!isShooting && !getIsDicking()) {
            ...
            if (getDirection() == LEFT_DIR) {
                bullet.setSpeedX(-10);     //   Tốc độ bay của đạn
                bullet.setPosX(bullet.getPosX() - 40);  //  set vị trí viên đạn sao cho khớp với nòng súng của animation
                if (getSpeedX() != 0 && getSpeedY() == 0) {
                    bullet.setPosX(bullet.getPosX() - 10);  //  Ở đây cũng là để set vị trí viên đạn khi đang chạy
